...
import { PythonDataSource } from 'lib-neuroglancer/datasource/python/frontend';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (window.location.hash) {
    const hashBinding = viewer.registerDisposer(
      new UrlHashBinding(viewer.state)
    );
  ...
  remoteActionHandler.sendActionRequested.add((action, state) =>
    serverConnection.sendActionNotification(action, state)
  );
